...
def execute_sysadminctl(domainname, passworddomain, sudopassword):
    if check_ad_connection():
        try:
            log("Starting sysadminctl command")
            
            # Disable secureToken
            cmd = ["sudo", "-S", "sysadminctl", "-secureTokenOff", domainname, "-password", passworddomain, "-adminUser", "it", "-adminPassword", localadminpassword]
            ...
            if "Incorrect password for" in result.stderr:
                ...
                # Enable secureToken
                cmd = ["sudo", "-S", "sysadminctl", "-secureTokenOn", domainname, "-password", passworddomain, "-adminUser", "it", "-adminPassword", localadminpassword]
                result = subprocess.run(cmd, capture_output=True, text=True, input=sudopassword + "\n")
...
def create_ui():
    ...
    def on_submit():
        ...
        if domainname and passworddomain and sudopassword:
            execute_sysadminctl(domainname, passworddomain, sudopassword)
        else:
