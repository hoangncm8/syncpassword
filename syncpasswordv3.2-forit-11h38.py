...
def secureTokenOff_button(domainname, domainpassword, sudo_password, username):
    """Disable secure token."""
    domainname = entry_domain_name.get()  # Store domain name in variable
    domainpassword = entry_domain_password.get()  # Store domain password in variable
    sudo_password = entry_sudo_password.get() # Store sudo password in variable
...
def check_secureToken_status(domainname):
...
root.geometry("440x220")  # Kích thước mới: 600 pixel chiều rộng và 400 pixel chiều cao
...
btn_check_network.grid(row=4, column=1, padx=10, pady=10)  # Bên trái

# SecureToken On button
btn_create = tk.Button(root, text="SecureToken On", command=secureTokenOn)
btn_create.grid(row=4, column=2,padx=10,pady=10)  # Bên phải
